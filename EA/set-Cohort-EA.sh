#!/bin/sh
#
# Edited and adopted by Trve Kelderman of District 65 (keldermant@district65.net)
...
if [ -f /Library/JAMF\ D65/Cohort/*.txt ]; then
  cohort=$(head -n 1 /Library/JAMF\ D65/Cohort/*.txt)
elif [-f /Library/JAMF\ D65/*.txt]; then
	cohort=$(head -n 1 /Library/JAMF\ D65/*.txt)
else
	cohort=""
fi
